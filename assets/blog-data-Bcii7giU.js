import{f as l}from"./marked.esm-DDYBFJNL.js";const t=e=>e.replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim(),u=e=>{const n=[],o=/<h2[^>]*>([^<]+)<\/h2>\s*<(p|ul|ol)[^>]*>([\s\S]*?)<\/(?:p|ul|ol)>/g;let a=o.exec(e);for(;a!==null;){const s=t(a[1]);if(s.includes("?")){const i=t(a[3]).slice(0,500);i&&n.push({question:s,answer:i})}a=o.exec(e)}return n},d=e=>e.replace(/^["']|["']$/g,""),p=(e,n)=>{const o=n.match(/^---\s*\n([\s\S]*?)\n---\s*\n?/),a={};if(o)for(const r of o[1].split(`
